...
namespace HouseFinancesAPI.Controllers.LogApp
{
    ...
    public class LogApplicationController(
        IServiceProvider service, 
        ILogApplicationRepository _logRepository) : BaseApiController(service)
    {
        [HttpGet]
        public async Task<PagedResult<LogRequest>> GetAllAsync(int paginaAtual = 1, int itensPorPagina = 10)
           => await Pagination.PaginateResultAsync(_logRepository.GetLogs(), paginaAtual, itensPorPagina);
    }
}
